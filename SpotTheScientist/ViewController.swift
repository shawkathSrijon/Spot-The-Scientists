...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    func renderer(_ renderer: SCNSceneRenderer, nodeFor anchor: ARAnchor) -> SCNNode? {
        ...
        let plain = SCNPlane(width: imageAnchor.referenceImage.physicalSize.width, height: imageAnchor.referenceImage.physicalSize.height)
        plain.firstMaterial?.diffuse.contents = UIColor.clear
        
        let plainNode = SCNNode(geometry: plain)
        ...
        titleNode.position.x += Float(plain.width/2) + spacing
        titleNode.position.y += Float(plain.height/2)
        
        plainNode.addChildNode(titleNode)
        
        let bioNode = textNode(scientist.bio, font: UIFont.systemFont(ofSize: 4), maxWidth: 100)
        bioNode.pivotOnTopLeft()
        bioNode.position.x += Float(plain.width/2) + spacing
        bioNode.position.y = titleNode.position.y - titleNode.height - spacing
        
        plainNode.addChildNode(bioNode)
        ...
        flagNode.position.y -= Float(plain.height / 2) + spacing
        
        plainNode.addChildNode(flagNode)
